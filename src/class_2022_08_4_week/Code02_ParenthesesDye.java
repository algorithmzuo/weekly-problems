...
public class Code02_ParenthesesDye {
	...
	// 这个递归用了记忆化搜索的方式实现了动态规划
	// dp就是动态规划表
	// f(arr, i, j)含义 :
	// arr[0...i]范围上，在保证任何前缀上，右括号都不超标的情况下
	// 返回arr[0...i]范围上，左括号比右括号多j个的方法数
	// 如果是如上的含义，那么f(arr, n-1, 0)就代表：
	// arr整体范围上，在保证任何前缀上，右括号都不超标的情况下
	// 返回整个范围上，左括号比右括号多0个的方法数
	// 这个含义就是arr变成合法括号结合的数量
	public static int f(int[] arr, int i, int j, int[][] dp) {
		...
		if (arr[i] > 0) {
			...
			// 0..i整体上，左括号要比右括号多j个
			// 当前i字符已经是左括号了
			// 那么就要求0...i-1上，左括号比右括号多j-1个
			ans = f(arr, i - 1, j - 1, dp);
		} else {
			// 当前i字符既可以是左括号、也可以是右括号
			// 0..i整体上，左括号要比右括号多j个
			// 如果当前i字符决定是左括号 -> f(arr, i - 1, j - 1)
			// 如果当前i字符决定是右括号 -> f(arr, i - 1, j + 1)
